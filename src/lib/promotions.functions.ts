...
export const listPromotions = createServerFn({ method: "GET" }).handler(
  async (): Promise<Cheese[]> => {
    ...
    do {
      const url = new URL(`${GATEWAY}/v0/${BASE_ID}/${encodeURIComponent(TABLE)}`);
      ...
      if (!res.ok) {
        ...
        // Table absente ou vide : on renvoie une liste vide plutôt que crasher.
